...
void AP_MotorsMatrix::output_armed_stabilizing()
{
    ...
    float   thr_adj;            // the difference between the pilot's desired throttle and throttle_thrust_best_rpy (the throttle that is actually provided)
    ...
    roll_thrust = calc_roll_thrust();
    pitch_thrust = calc_pitch_thrust();
    yaw_thrust = calc_yaw_thrust();
    throttle_thrust = calc_throttle_thrust();
    ...
    for (i=0; i<AP_MOTORS_MAX_NUM_MOTORS; i++) {
        if (motor_enabled[i]) {
            rpy_out[i] = roll_thrust * _roll_factor[i] * get_compensation_gain() +
                            pitch_thrust * _pitch_factor[i] * get_compensation_gain();
    ...
    if (yaw_thrust >= 0.0f) {
        // if yawing right
        if (yaw_allowed > yaw_thrust * get_compensation_gain()) {
            yaw_allowed = yaw_thrust * get_compensation_gain(); // to-do: this is bad form for yaw_allows to change meaning to become the amount that we are going to output
        }else{
            limit.yaw = true;
        }
    }else{
    ...
    for (i=0; i<AP_MOTORS_MAX_NUM_MOTORS; i++) {
        if (motor_enabled[i]) {
            motor_out[i] = throttle_thrust_best_rpy+thr_adj +
                            rpy_scale*rpy_out[i];
        }
    }
    ...
    for (i=0; i<AP_MOTORS_MAX_NUM_MOTORS; i++) {
        if (motor_enabled[i]) {
            motor_out[i] = calc_thrust_to_pwm(motor_out[i]);
        }
    }

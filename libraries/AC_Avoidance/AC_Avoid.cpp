...
void AC_Avoid::adjust_velocity_proximity(const float kP, const float accel_cmss, Vector2f &desired_vel)
{
    ...
    // convert desired-vel vector into an body-frame angle
    Vector2f vel_dir = desired_vel.normalized();
    Vector2f vel_dir_bf(vel_dir.x*_ahrs.cos_yaw()+vel_dir.y*_ahrs.sin_yaw(), -vel_dir.x*_ahrs.sin_yaw()+vel_dir.y*_ahrs.cos_yaw());

    float heading_rad = vel_dir.angle(Vector2f(0,0));
    float heading_bf_rad = wrap_PI(_ahrs.yaw - heading_rad);
    uint16_t distance_cm;
    if (_proximity.get_horizontal_distance(degrees(heading_bf_rad), distance_cm)) {
        // shorten desired velocity
        limit_velocity(kP, accel_cmss, desired_vel, vel_dir, distance_cm);
    }
}

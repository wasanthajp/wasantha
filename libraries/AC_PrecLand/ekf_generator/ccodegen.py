...
def getSnippetDefines(opname, funcs):
    ...
    for retName in sorted(funcs.keys()):
        func = funcs[retName]
        ...
        for paramname in sorted(func['params'].keys()):
            ...
            if (nr,nc) == (1,1):
                substitutions += list(zip(func['params'][paramname], Matrix(nr,nc, [Symbol(paramlist[-1])])))
            elif nc == 1:
                substitutions += list(zip(func['params'][paramname], Matrix(nr,nc, symbols(paramlist[-1]+'[0:%u]'%(nr,)))))
            else:
                substitutions += list(zip(func['params'][paramname], Matrix(nr,nc, symbols(paramlist[-1]+'[0:%u][0:%u]'%(nr,nc)))))
        ...
        if 'retsymbols' in func:
            ...
            if (nr,nc) == (1,1):
                substitutions += list(zip(func['retsymbols'], Matrix(nr,nc, [Symbol(retParamName)])))
            elif nc == 1:
                substitutions += list(zip(func['retsymbols'], Matrix(nr,nc, symbols(retParamName+'[0:%u]'%(nr,)))))
            else:
                substitutions += list(zip(func['retsymbols'], Matrix(nr,nc, symbols(retParamName+'[0:%u][0:%u]'%(nr,nc)))))
...
def double2float(string):
    import re
    string = re.sub(r"[0-9]+\.[0-9]+(e[-+]?[0-9]+)?", '\g<0>f', string)

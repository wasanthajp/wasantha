...
*/
AP_Proximity_LightWareSF40C::AP_Proximity_LightWareSF40C(Proximity &_ranger, uint8_t instance,
                                                               Proximity::Proximity_State &_state,
                                                               AP_SerialManager &serial_manager) :
    AP_Proximity_Backend(_ranger, instance, _state)
{
...
bool AP_Proximity_LightWareSF40C::detect(Proximity &_ranger, uint8_t instance, AP_SerialManager &serial_manager)
{
    return serial_manager.find_serial(AP_SerialManager::SerialProtocol_Lidar, 0) != nullptr;
}

// get distance in cm in a particular direction in degrees (0 is forward, angles increase in the clockwise direction)
bool AP_Proximity_LightWareSF40C::get_horizontal_distance(int16_t angle_deg, int16_t &distance_cm)
{
    uint8_t sector;
...
void AP_Proximity_LightWareSF40C::update(void)
{
    ...
    if ((_last_distance_received_ms == 0) || (AP_HAL::millis() - _last_distance_received_ms > PROXIMITY_SF40C_TIMEOUT_MS)) {
        set_status(Proximity::Proximity_NoData);
    } else {
        set_status(Proximity::Proximity_Good);
    }
}
...
bool AP_Proximity_LightWareSF40C::convert_angle_to_sector(float angle_degrees, uint8_t &sector)
{
    // sanity check angle

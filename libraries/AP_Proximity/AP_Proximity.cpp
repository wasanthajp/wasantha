...
const AP_Param::GroupInfo AP_Proximity::var_info[] = {
    ...
    // @User: Standard
    AP_GROUPINFO("_TYPE",    2, AP_Proximity, _type[0], 0),
...
bool AP_Proximity::get_horizontal_distance(uint8_t instance, int16_t angle_deg, uint16_t &distance_cm) const
{
...
// get distance in cm in a particular direction in degrees (0 is forward, clockwise)
// returns true on successful read and places distance in distance_cm
bool AP_Proximity::get_horizontal_distance(int16_t angle_deg, uint16_t &distance_cm) const
{
    return get_horizontal_distance(primary_instance, angle_deg, distance_cm);
}

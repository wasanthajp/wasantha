...
void AP_Stats::update_flighttime()
{
    if (_flying_ms) {
        ...
        const uint16_t delta = (now - _flying_ms)/1000;
        flttime += delta;
...
{
    const uint32_t now = AP_HAL::millis();
    const uint16_t delta = (now - _last_runtime_ms)/1000;
    runtime += delta;
...
void AP_Stats::update()
{
    ...
    const uint32_t param_reset = params.reset;
    if (param_reset != reset) {
        params.bootcount.set_and_save(0);
...
void AP_Stats::set_flying(const bool b)
{
    if (b) {
        if (!_flying_ms) {

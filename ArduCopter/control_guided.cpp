...
bool Copter::FlightController_GUIDED::takeoff_start(float final_alt_above_home)
{
...
void Copter::FlightController_GUIDED::pos_control_start()
{
...
void Copter::FlightController_GUIDED::vel_control_start()
{
...
void Copter::FlightController_GUIDED::posvel_control_start()
{
    // set guided_mode to velocity controller
...
void Copter::FlightController_GUIDED::angle_control_start()
{
    // set guided_mode to velocity controller
...
void Copter::FlightController_GUIDED::set_destination(const Vector3f& destination)
{
    ...
    // no need to check return status because terrain data is not used
    _copter.wp_nav.set_wp_destination(destination, false);
...
bool Copter::FlightController_GUIDED::set_destination(const Location_Class& dest_loc)
{
    // ensure we are in position control mode
    if (guided_mode != Guided_WP) {
        pos_control_start();
...
void Copter::FlightController_GUIDED::set_velocity(const Vector3f& velocity)
{
    ...
    _copter.pos_control.set_desired_velocity(velocity);
...
void Copter::FlightController_GUIDED::set_destination_posvel(const Vector3f& destination, const Vector3f& velocity) {
    // check we are in velocity control mode
    ...
    _copter.pos_control.set_pos_target(posvel_pos_target_cm);
...
void Copter::FlightController_GUIDED::set_angle(const Quaternion &q, float climb_rate_cms)
{
    // check we are in velocity control mode
...
void Copter::FlightController_GUIDED::run()
{
...
void Copter::FlightController_GUIDED::takeoff_run()
{
...
void Copter::FlightController_GUIDED::pos_control_run()
{
    // if not auto armed or motors not enabled set throttle to zero and exit immediately
...
// called from guided_run
void Copter::FlightController_GUIDED::vel_control_run()
{
    // if not auto armed or motors not enabled set throttle to zero and exit immediately
    if (!motors.armed() || !ap.auto_armed || !motors.get_interlock() || ap.land_complete) {
        // initialise velocity controller
    ...
    pos_control.update_vel_controller_xyz(_copter.ekfNavVelGainScaler);
    ...
    if (_copter.auto_yaw_mode == AUTO_YAW_HOLD) {
        ...
        // roll, pitch from waypoint controller, yaw heading from auto_heading()
        attitude_control.input_euler_angle_roll_pitch_yaw(pos_control.get_roll(), pos_control.get_pitch(), _copter.get_auto_heading(), true);
    }
}

// guided_posvel_control_run - runs the guided spline controller
// called from guided_run
void Copter::FlightController_GUIDED::posvel_control_run()
{
    // if not auto armed or motors not enabled set throttle to zero and exit immediately
    if (!motors.armed() || !ap.auto_armed || !motors.get_interlock() || ap.land_complete) {
        // set target position and velocity to current position and velocity
    ...
    if (dt >= pos_control.get_dt_xy()) {
        ...
        // run position controller
        pos_control.update_xy_controller(AC_PosControl::XY_MODE_POS_AND_VEL_FF, _copter.ekfNavVelGainScaler, false);
    }
...
// called from guided_run
void Copter::FlightController_GUIDED::angle_control_run()
{
    // if not auto armed or motors not enabled set throttle to zero and exit immediately
    if (!motors.armed() || !ap.auto_armed || !motors.get_interlock() || ap.land_complete) {
#if FRAME_CONFIG == HELI_FRAME  // Helicopters always stabilize roll/pitch/yaw
    ...
    pos_control.set_alt_target_from_climb_rate_ff(climb_rate_cms, _copter.G_Dt, false);
    pos_control.update_z_controller();
}
...
void Copter::FlightController_GUIDED::limit_clear()
{
...
void Copter::FlightController_GUIDED::limit_set(uint32_t timeout_ms, float alt_min_cm, float alt_max_cm, float horiz_max_cm)
{
...
void Copter::FlightController_GUIDED::limit_init_time_and_pos()
{
...
bool Copter::FlightController_GUIDED::limit_check()
{

...
bool Copter::FlightController_FLIP::init(bool ignore_checks)
{
    ...
    flip_orig_control_mode_reason = _copter.control_mode_reason;
    ...
    // log start of flip
    _copter.Log_Write_Event(DATA_FLIP_START);
...
void Copter::FlightController_FLIP::run()
{
    float throttle_out;
    ...
    case Flip_Recover:
        ...
        if (fabsf(recovery_angle) <= FLIP_RECOVERY_ANGLE) {
            ...
            if (!_copter.set_mode(flip_orig_control_mode, flip_orig_control_mode_reason)) {
                // this should never happen but just in case
                _copter.set_mode(STABILIZE, MODE_REASON_UNKNOWN);
            }
            // log successful completion
            _copter.Log_Write_Event(DATA_FLIP_END);
        }
        break;

...
static bool event_send_strings = true;     // true to be backwards compatible and send strings along with events
...
static void event_send_and_record(uint8_t event_id, uint8_t set_or_cleared)
{
    ...
    // send strings as well to GCS
    if ((set_or_cleared == EVENT_SET) && event_send_strings) {
        struct EventInfo evinf;
        if (event_get_info(event_id, evinf)) {
            gcs_send_text_P(evinf.severity, evinf.description);
        }
    }
}
...
static bool event_handle_request_event_info(mavlink_channel_t chan, mavlink_message_t* msg)
{
    ...
    // GCS capable of comprehending event messages so disable sending strings
    event_send_strings = false;
